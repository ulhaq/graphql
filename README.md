...
However, you will soon see, that we only have one endpoint for both `Query` and `Mutation` operations in GraphQL.######## Also, all HTTP requests in GraphQL are done with `POST` - at least those we will be looking at today.
